...
class MenuOptionSmokeTests(unittest.IsolatedAsyncioTestCase):
    async def test_production_menu_options_do_not_raise(self):
        ...
        common_patches = (
            ...
            patch("pulse.integrations.production._continue_batch_creation_actor_selection", new=AsyncMock(return_value=True)),
            ...
            patch("pulse.integrations.production._show_batch_owner_selection", new=AsyncMock()),
            patch("pulse.integrations.production._show_batch_notifier_selection", new=AsyncMock()),
        ...
        cases = [
            ...
            (
                production.SELECTING_BATCH_OWNER_STATE,
                "1",
                {"batch_owner_selection": {"candidates": [{"costing_ref": 1, "user_id": "U1", "name": "Sup 1"}]}},
            ),
            (
                production.SELECTING_BATCH_NOTIFIERS_STATE,
                "1",
                {"batch_notifier_selection": {"candidates": [{"costing_ref": 2, "user_id": "U2", "name": "Sup 2"}]}},
            ),
